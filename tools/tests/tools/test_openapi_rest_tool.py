    ...
    def request(self, method, url, headers=None, params=None, json=None, timeout=None):
        ...
        if url == "https://api.example.com/data":
            if headers and headers.get("Authorization") == "Bearer my_test_token":
                if method == "GET":
                    return Response(
                        200, request=req, json={"success": True, "message": "GET successful"}
                    )
                if method == "POST" and json == {"name": "test"}:
                    return Response(
                        201, request=req, json={"success": True, "message": "POST successful"}
                    )
            else:
...
def test_openapi_rest_tool_post(mcp, credentials, monkeypatch):
    ...
    result = openapi_request(
        method="POST", url="https://api.example.com/data", json_body={"name": "test"}
    )
...
def test_openapi_rest_tool_security(mcp, credentials, monkeypatch):
    register_openapi_rest(mcp, credentials=credentials)
    openapi_request = mcp._tool_manager._tools["openapi_request"].fn

    result = openapi_request(method="GET", url="https://malicious.com/data")
