...
class EncryptedFileStorage(CredentialStorage):
    ...
    def rotate_key(self, new_key: bytes, audit_log_path: Path | None = None) -> dict:
        ...
        Args:
            ...
            audit_log_path: Optional path to write rotation audit log

        Returns:
            Dictionary with rotation results:
            {
                "rotated": ["cred1", "cred2"],
                "failed": {"cred3": "error message"},
                "total": 10,
                "success": 9,
        ...
        results["total"] = len(all_creds)

        for credential_id in all_creds:
            try:
                # Load credential (decrypt with old key)
                cred_path = self._cred_path(credential_id)
                with open(cred_path, "rb") as f:
                    encrypted = f.read()

                json_bytes = old_fernet.decrypt(encrypted)
                ...
                results["rotated"].append(credential_id)
                results["success"] += 1

            except Exception as e:
                results["failed"][credential_id] = str(e)
                logger.error(f"Failed to rotate key for '{credential_id}': {e}")

        # Update instance to use new key
        self._key = new_key
        self._fernet = new_fernet

        # Also persist the new key to file if we are using auto-generated keys
        key_file = self.base_path / ".encryption_key"
        if key_file.exists():
            key_file.write_bytes(new_key)
            logger.info(f"Updated persisted encryption key at {key_file}")
        ...
        logger.info(
            f"Key rotation complete: {results['success']}/{results['total']} credentials rotated, "
