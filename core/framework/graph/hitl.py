...
class HITLProtocol:
    ...
    def parse_response(
        ...
        if not use_haiku or not api_key:
            ...
            logger.info(
                "Skipping Haiku parsing (use_haiku=%s, api_key_present=%s). "
                "Falling back to raw input for the first question.",
                use_haiku,
                bool(api_key),
            )
...
{{"question-1": "answer here", "question-2": "answer here"}}"""
            ...
            if json_match:
                ...
                logger.warning("No JSON object found in Haiku's response: %s", response_text)

        except Exception:
            logger.exception(
                "Failed to parse response with Haiku. "
                "Falling back to raw input for the first question."
            )
