...
def _handle_judge_failure(
    ...
    Args:
        phase_name: The name of the phase being evaluated.
        failure_count: Number of consecutive previous judge failures.
        max_failures: Maximum allowed consecutive failures before degrading to ACCEPT.
        reason: Description of the failure reason.

    Returns:
        PhaseVerdict with either RETRY or ACCEPT depending on failure count.
    """
    ...
    if attempt >= max_failures:
        ...
        return PhaseVerdict(
            ...
            confidence=0.0,
            feedback=(
                "[SYSTEM] Quality gate skipped — judge LLM failed "
                f"{attempt} consecutive times ({reason})."
            ),
        )
    logger.warning(
