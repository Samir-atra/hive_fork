...
If they've already provided the transcript in their message, \
acknowledge it and ask for the optional details.
...
extract_node = NodeSpec(
    ...
    system_prompt="""\
You are a professional meeting analyst. Parse the provided meeting transcript \
and extract structured information.

**EXTRACTION RULES:**

1. **Summary**: Write a concise 2-3 sentence executive summary of the \
meeting's purpose and outcomes.

2. **Attendees**: List all people mentioned in the transcript with their \
roles if stated. Format: "Name (Role)" or just "Name".

3. **Decisions**: Extract all key decisions that were agreed upon during \
the meeting. Each decision should be a clear statement.
...
5. **Blockers**: Unresolved issues that are preventing progress. Must be \
explicitly mentioned as problems/blockers/issues.

6. **Follow-ups**: Items needing future attention but not yet formally assigned \
as action items.
...
- set_output("action_items", [{"task": "...", "owner": "...", "due": "...", \
...
review_node = NodeSpec(
    ...
    success_criteria=("The user has reviewed the extracted data and confirmed it is accurate."),
    system_prompt="""\
...
deliver_node = NodeSpec(
    ...
    success_criteria=(
        "Meeting notes are saved to a file and optionally posted to Slack. "
...
- Call slack_post_message(channel={{slack_channel}}, text=<formatted message>)
