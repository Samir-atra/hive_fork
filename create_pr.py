...
try:
    ...
    with open('.pr-6639.md', 'r') as f:
        body = f.read()
    # Create PR within the fork: head is the branch name in the same repo
    pr = api.pulls.create(
        title="fix: add explicit prompt for auto-blocking",
        head="feat/add-queen-auto-block-prompt-6639",  # Branch in Samir-atra/hive_fork
        base="main",
        body=body,
    ...
    print(f"Failed to create PR (expected if no auth): {e}")
