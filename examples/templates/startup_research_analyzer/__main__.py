"""
CLI entry point for Startup Research Analyzer.
...
from .agent import default_agent, StartupResearchAnalyzer
...
def cli():
    """Startup Research Analyzer - Interactive, rigorous research with TUI conversation."""
    pass
...
def tui(verbose, debug):
    ...
    async def run_with_tui():
        agent = StartupResearchAnalyzer()

        # Build graph and tools
        agent._event_bus = EventBus()
        agent._tool_registry = ToolRegistry()

        storage_path = Path.home() / ".hive" / "agents" / "startup_research_analyzer"
...
async def _interactive_shell(verbose=False):
    ...
    click.echo("=== Startup Research Analyzer ===")
    ...
    try:
        while True:
            try:
                ...
                result = await agent.trigger_and_wait("default", {"topic": topic})
